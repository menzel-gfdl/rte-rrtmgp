...
integer, parameter, public :: namelen = 32 !< Length of argument names.
integer, parameter, public :: valuelen = 512 !< Length of argument values.


!> @brief Argument type.
type, private :: Argument_t
  character(len=namelen) :: short_name !< Short name (must start with a - if it is optional).
  character(len=namelen) :: long_name !< Long name (must start with a --).
  character(len=valuelen) :: description !< Description that will appear in the help message.
  character(len=valuelen) :: val !< Buffer where the input value is stored.
  logical :: positional !< Flag telling if the argument is positional.
  logical :: requires_val !< Flag telling if the argument requires a value.
...
function create_parser() result(p)
  type(Parser_t) :: p
  p%args => null()
  p%argc = 0
  p%num_pos_args = 0
...
subroutine add_argument(parser, short_name, description, requires_val, long_name)
  ...
  logical, intent(in), optional :: requires_val !< Does the option require a value?
